...
function getEmployeeApproved(){
    ...
    fetch(
        'http://localhost:9001/Project1/ajax/empapproved')   
        .then(function(responseObject) {
            ...
            ourApprovedDOMManipulation(daSecondResponse);
        })
}

function ourApprovedDOMManipulation(ourJSON) {
	...
	for (let i = 0; i < ourJSON.length; i++) {
		...
		let newSelectionTwo = document.querySelector("#approvedTableBody");
